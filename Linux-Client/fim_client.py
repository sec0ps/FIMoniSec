...
try:
    ...
    fim_integration.initialize()  # Initialize right after creating the instance
    print("[INFO] Enhanced FIM capabilities loaded")
except ImportError as e:
    print(f"[INFO] Running with basic FIM capabilities: {e}")
...
def log_event(event_type, file_path, previous_metadata=None, new_metadata=None, previous_hash=None, new_hash=None, changes=None):
    ...
    # Process event with enhanced FIM if available
    try:
        if 'fim_integration' in globals() and hasattr(fim_integration, 'process_enhanced_event'):
            enhanced_result = fim_integration.process_enhanced_event(log_entry)
            if enhanced_result:
                # Merge any enhanced analysis results
                if 'enhanced_analysis' in enhanced_result:
                    log_entry['enhanced_analysis'] = enhanced_result['enhanced_analysis']
                
                # Add any alerts generated by enhanced modules
                if 'enhanced_alerts' in enhanced_result:
                    log_entry['enhanced_alerts'] = enhanced_result['enhanced_alerts']
                    
                    # Log high-risk alerts to console
                    if enhanced_result.get('high_risk', False):
                        alert_reasons = enhanced_result.get('alert_reasons', ["High risk score"])
                        print(f"[ENHANCED ALERT] High-risk event detected: {file_path}")
                        print(f"[ENHANCED ALERT] Reasons: {', '.join(alert_reasons)}")
    except Exception as e:
        print(f"[WARNING] Enhanced event processing failed: {e}")
...
def run_monitor():
    ...
    # Initialize enhanced FIM if available and not already initialized
    try:
        if 'fim_integration' in globals():
            if not fim_integration.initialized:  # Check if already initialized
                fim_integration.initialize(config)
                print("[INFO] Enhanced FIM capabilities initialized")
            else:
                print("[INFO] Enhanced FIM already initialized")
    except Exception as e:
        print(f"[WARNING] Failed to initialize enhanced FIM: {e}")
...
def handle_shutdown(signum=None, frame=None):
    ...
    # Shutdown enhanced FIM components if available
    try:
        if 'fim_integration' in globals():
            fim_integration.shutdown()
            print("[INFO] Enhanced FIM components shut down.")
    except Exception as e:
        print(f"[WARNING] Failed to cleanly shut down enhanced FIM: {e}")
